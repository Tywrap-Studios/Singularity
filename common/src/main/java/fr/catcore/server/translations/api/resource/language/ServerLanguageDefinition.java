...
public record ServerLanguageDefinition(String code, String region, String name,
    ...
    public static final ServerLanguageDefinition DEFAULT = new ServerLanguageDefinition(DEFAULT_CODE, "US", "English",
            false);

    public static Pair<List<ServerLanguageDefinition>, Map<String, String>> loadLanguageDefinitions()
            throws IOException {
        List<ServerLanguageDefinition> languageDefinitions = new ArrayList<>();
        Map<String, String> aliasList = new HashMap<>();

        try (BufferedReader read = new BufferedReader(
                new InputStreamReader(ServerTranslations.class.getResourceAsStream("/minecraft_languages.json")))) {
            ...
            for (Map.Entry<String, JsonElement> entry : languageRoot.entrySet()) {
                ServerLanguageDefinition definition = ServerLanguageDefinition.parse(entry.getKey(),
    ...
    public boolean equals(Object o) {
        if (this == o) {
            ...
            return o instanceof ServerLanguageDefinition languageDefinition
                    && this.code.equals(languageDefinition.code);
        }
    }
